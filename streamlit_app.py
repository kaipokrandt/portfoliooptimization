...
def backtest_rolling(prices: pd.DataFrame, lookback_days: int = 252, rebalance_months: int = 1, strategy: str = 'max_sharpe'):
    ...
    returns = prices.pct_change().dropna()
    dates = returns.index  # iterate over returns, not prices
    ...
    for i, current_date in enumerate(dates):
        ...
        if current_weights is not None:
            # use returns aligned with current weights
    ...
    return portfolio_daily.fillna(method='ffill').fillna(1.0)
...
if run:
    ...
    if prices.shape[1] < 2:
        ...
        bt_max = backtest_rolling(prices, lookback_days=252, rebalance_months=rebalance_months, strategy='max_sharpe')
        bt_min = backtest_rolling(prices, lookback_days=252, rebalance_months=rebalance_months, strategy='min_volatility')
